...
if(!isset($_SESSION['auth']) || $_SESSION['auth']!=1){
    ...
    if($secret_str=='' && (ereg("[[:punct:]]",$login) || ereg("[[:punct:]]",$password))) {
    ...
    } elseif ($secret_str=='' && ($password=="" || $login=="") ) {
	    ...
	    where u.login like "'.$login.'"';
                ...
                // это самое то место с авторизацией
                ...
                // попробуем авторизовать через лдап
                $ldap = ldap_connect("192.168.65.128");
                if ($ldap) {
                    // попробуем найти в каталоге пользователя с указанным имененм
                    // $sr = ldap_search($ds, "ou=asu,dc=example,dc=com", "userid=".$login);

                    // попробуем войти в ldap с указанным именем и паролем
                    ldap_set_option($ldap, LDAP_OPT_PROTOCOL_VERSION, 3);
                    $sr = ldap_bind($ldap, "userid=".$login.",ou=asu,dc=example,dc=com", $password);
                    var_dump($sr);
